...
fn main() {
    ...
    {
        let db = DB::new("single_trie_writes_db", &cfg.clone().truncate(true).build()).unwrap();
        let items = vec![
            ("do", "verb"),
            ("doe", "reindeer"),
            ("dog", "puppy"),
    ...
    {
        let db = DB::new("single_trie_writes_db", &cfg.clone().truncate(false).build()).unwrap();
        //println!("{}\n{}", hex::encode(&*db.root_hash()), db.dump());
        let mut wb = db.new_writebatch();
    ...
    {
        let db = DB::new("single_trie_writes_db", &cfg.truncate(false).build()).unwrap();
        println!("{}\n{}", hex::encode(&*db.root_hash()), db.dump());
    }
}
