...
use clap::Parser;
use std::{collections::HashMap, error::Error, ops::RangeInclusive, time::Instant};

use firewood::db::{Batch, BatchOp, Db, DbConfig, DbError};
...
fn main() -> Result<(), DbError> {
    ...
    let db = Db::new_internal("rev_db", cfg).expect("db initiation should succeed");
...
fn verify_keys(db: &Db, verify: HashMap<Vec<u8>, Vec<u8>>) -> Result<(), DbError> {
    if !verify.is_empty() {
        let hash = db.kv_root_hash()?;
        let revision = db.get_revision(&hash).unwrap();
        for (key, value) in verify {
            assert_eq!(Some(value), revision.kv_get(key));
        }
    }
